test_that("project_pm() returns a bpm_proj_pm object", {
  fit  <- bpm(y ~ x1 + x2, data = toy, prior = flat())
  proj <- project_pm(fit)
  expect_s3_class(proj, "bpm_proj_pm")
})
...
test_that("predict.bpm_proj_pm type='response' returns values in (0, 1)", {
  proj <- project_pm(bpm(y ~ x1 + x2, data = toy, prior = flat()))
  p    <- predict(proj, new1)
  expect_true(p > 0 && p < 1)
})

test_that("predict.bpm_proj_pm type='link' returns a finite numeric", {
  proj <- project_pm(bpm(y ~ x1 + x2, data = toy, prior = flat()))
  eta  <- predict(proj, new1, type = "link")
  expect_true(is.numeric(eta) && is.finite(eta))
})

test_that("predict.bpm_proj_pm is close to predict(fit, method='pm') with large n", {
  fit    <- bpm(y ~ x1 + x2, data = toy, prior = flat())
  proj   <- project_pm(fit)
...
test_that("predict.bpm_proj_pm handles multiple rows", {
  proj   <- project_pm(bpm(y ~ x1 + x2, data = toy, prior = flat()))
...
test_that("print.bpm_proj_pm runs without error", {
  proj <- project_pm(bpm(y ~ x1 + x2, data = toy, prior = flat()))
