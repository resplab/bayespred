...
`bpmproj_pm()` regresses the posterior-mean soft labels onto a design matrix,
producing a standalone `bpmproj_pm` object — a simplified deployable model
with no covariance matrix. All three aspects default to the main fit but can
be overridden: `formula` (predictor set), `family` (link function), and `data`
(can be the development sample, local site data, or any external dataset).
...
The `bpmproj_pm` object is fully self-contained and requires only base R at
deployment time.
...
For multi-centre or federated settings, `likelihood()` returns the unpenalised
MLE and observed Fisher information — the data's contribution independent of
the prior. `posterior()` returns the MAP estimate and Laplace-approximated
covariance used for prediction.
